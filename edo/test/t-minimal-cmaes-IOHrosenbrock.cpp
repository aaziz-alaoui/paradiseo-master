...
int main(int argc, char** argv) {
    ...
    rosenbrock.IOHprofiler_set_number_of_variables(dim);
    IOHprofiler_csv_logger<R::AtomType> logger("./","run_problem_rosenbrock","CMA-ES","CMA-ES");

    logger.set_complete_flag(true);
    logger.set_interval(0);

    logger.activate_logger();
    logger.track_problem(rosenbrock.IOHprofiler_get_problem_id(), 
                        rosenbrock.IOHprofiler_get_number_of_variables(), 
                        rosenbrock.IOHprofiler_get_instance_id(),
                        rosenbrock.IOHprofiler_get_problem_name(),
                        rosenbrock.IOHprofiler_get_optimization_type());

     
    auto& obj_func = state.pack< eoEvalIOHproblem<R> >(rosenbrock, logger);
